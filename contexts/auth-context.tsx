...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const loadUserFromStorage = () => {
      ...
      try {
        console.log("=== CHARGEMENT DEPUIS STORAGE ===")
  ...
  const updateUserData = (userData: Partial<User>) => {
    setUser(prevUser => {
      ...
      // Mettre à jour le localStorage (seulement côté client)
  ...
  const isAuthenticated = () => {
    // Côté serveur, retourner false par défaut
    if (typeof window === 'undefined') {
      return false
    }
    ...
    // Si pas de cookie, essayer localStorage (seulement côté client)
    if (!userCookie && typeof window !== 'undefined' && window.localStorage) {
